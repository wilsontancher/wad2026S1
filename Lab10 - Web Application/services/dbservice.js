...
let db = {
    ...
    async updateEventById(id,name, description, startDate, startTime, endDate, endTime, organizerId) {
        try {
            let result = await event.findByIdAndUpdate(id, {
                name: name,
                description: description,
                start: {
                    date: startDate,
    ...
    async searchEvents(name) {
        try {
            let results = await event.find({ name: new RegExp(name,'i') }).populate('organizer', ['name','company']);
            return results;
        }
        catch(e) {
            console.log(e.message);
